...
export default function Home() {
    ...
    try {
      ...
      const interval = setInterval(() => {
        ...
        if (diff <= 0) {
          ...
          setLoading(false);
          // Validar si el usuario conectado es el ganador
          if (currentWallet?.toLowerCase() === ultimoB.toLowerCase()) {
            setIsWinner(true);
          }
  ...
  const conectarBilletera = async () => {
    ...
    if (typeof eth !== "undefined") {
      try {
        ...
        // Recargar datos con la nueva billetera conectada
        cargarDatos(account);
  ...
  const ejecutarCompra = async () => {
    ...
    try {
      ...
      await tx.wait(); // Esperamos que se confirme en la blockchain
      
      // ACTUALIZAMOS SIN RECARGAR LA PÁGINA
      await cargarDatos(wallet);
      setIsBuying(false);
    } catch (e: any) { 
      setIsBuying(false); 
      alert("Operación cancelada o saldo insuficiente.");
    }
  };
        ...
        <div className="bg-[#0A0A0A] border border-white/10 rounded-[40px] md:rounded-[60px] p-6 md:p-16 shadow-[0_0_50px_rgba(0,0,0,1)] relative overflow-hidden max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mb-12 md:mb-16">
            ...
            <div className="bg-gradient-to-b from-white/[0.04] to-transparent border border-white/5 rounded-[30px] md:rounded-[45px] p-8 md:p-12 text-center flex flex-col justify-center">
              ...
              {loading || !timeObj ? (
                <div className="text-xl md:text-3xl font-mono text-gray-600 animate-pulse uppercase tracking-widest">Sincronizando...</div>
              ) : isFinished ? (
                ...
                <div className="flex justify-center gap-2 md:gap-4">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="bg-black border border-white/10 rounded-xl md:rounded-2xl w-14 h-16 md:w-20 md:h-24 flex items-center justify-center shadow-[inset_0_0_20px_rgba(245,158,11,0.05)]">
                      <span className="text-3xl md:text-5xl font-black font-mono text-white">{timeObj.d.toString().padStart(2, '0')}</span>
                    </div>
          ...
          {/* BOTÓN DE ACCIÓN / GANADOR */}
          {isWinner ? (
            <button className="w-full py-6 md:py-8 bg-green-600 text-white rounded-[25px] md:rounded-[35px] font-black text-sm md:text-[16px] uppercase tracking-[0.3em] md:tracking-[0.5em] animate-pulse shadow-[0_0_40px_rgba(22,163,74,0.6)] mb-10 md:mb-14">
              RECLAMAR BÓVEDA AHORA
            </button>
          ) : (
            <button onClick={ejecutarCompra} disabled={isBuying || isFinished} className="w-full py-6 md:py-8 bg-white text-black rounded-[25px] md:rounded-[35px] font-black text-sm md:text-[16px] uppercase tracking-[0.3em] md:tracking-[0.5em] hover:bg-amber-500 hover:text-white transition-all shadow-[0_0_40px_rgba(255,255,255,0.2)] hover:shadow-[0_0_40px_rgba(245,158,11,0.6)] active:scale-95 mb-10 md:mb-14">
              {isFinished ? "BÓVEDA FINALIZADA" : isBuying ? "CONFIRMANDO PAGO..." : `INGRESAR AL POZO (~$${ticketUsd})`}
            </button>
          )}

          {/* LINK PÚBLICO DE BASESCAN (NUEVA CERTEZA) */}
          <div className="mb-14 p-6 bg-white/[0.02] rounded-3xl border border-white/5 inline-block">
            <a href={`https://basescan.org/address/${contractAddress}`} target="_blank" className="text-[10px] md:text-xs font-black text-gray-400 hover:text-amber-500 transition-colors uppercase tracking-widest flex items-center justify-center gap-3">
              <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span> 
              Ver transacciones en vivo (Basescan)
            </a>
          </div>
